...
const starterSteps = [
  ["01", "문제를 한 문장으로 적기", "무엇을 줄이고 싶은지부터 적습니다. 예: 매주 반복되는 거래처 보고서 초안 작성 시간을 줄이고 싶다. 번호와 표로 정리하고, 모르는 내용은 추측하지 말고 확인 필요로 표시해 줘. 이 요구사항으로 예쁜 반응형 React 웹앱을 기획·제작하세요. 입력 폼(여행지·마트·예산·구매자 조건), 카테고리 필터, 상품 카드, 예산 합계, 장바구니, 확인 필요 배지를 만들고 샘플 데이터로 실행 가능한 미리보기를 보여 주세요. 모던한 색상·여백·모바일 화면을 적용하고 수정할 파일과 실행 방법도 안내하세요."],
  ["02", "결과물을 먼저 정하기", "메일, 표, 보고서, 슬라이드, 업무일지처럼 마지막에 필요한 모양을 정합니다. 번호와 표로 정리하고, 모르는 내용은 추측하지 말고 확인 필요로 표시해 줘. 이 요구사항으로 예쁜 반응형 React 코디 웹앱을 제작하세요. 날씨·체형·TPO 입력 화면, 코디 3개 카드, 옷 상세, 아우터 대안, 저장·다시 추천 버튼을 만들고 샘플 데이터로 클릭 가능한 미리보기를 보여 주세요. 모바일 우선 디자인과 실행 방법을 함께 제공하세요."],
  ["03", "작은 자료로 시험하기", "민감정보를 지운 샘플 3~5건으로 먼저 실행하고 결과를 사람의 눈으로 비교합니다. 번호와 표로 정리하고, 모르는 내용은 추측하지 말고 확인 필요로 표시해 줘. 이 요구사항으로 안전한 생활 문제 해결 웹앱을 제작하세요. 문제·소재·준비물 입력, 단계별 타임라인, 금지 조합 경고, 중단 조건, 체크 완료 표시, 전문가 문의 버튼을 만들고 위험한 조언은 확인 필요로 표시하세요. React 반응형 UI와 샘플 실행 화면을 제공하세요."],
  ["04", "검수 기준을 만들기", "빠진 내용, 숫자 오류, 출처, 말투, 개인정보 노출 여부를 체크리스트로 만듭니다. 번호와 표로 정리하고, 모르는 내용은 추측하지 말고 확인 필요로 표시해 줘. 이 요구사항으로 스포일러 없는 콘텐츠 추천 웹앱을 제작하세요. 취향·시간·플랫폼·피할 요소 입력, 필터, 추천 카드, 비교표, 찜 버튼, 확인 필요 배지를 만들고 실제 서비스 여부는 사용자가 확인하도록 표시하세요. 예쁜 반응형 React 미리보기와 실행 방법을 제공하세요."],
];
